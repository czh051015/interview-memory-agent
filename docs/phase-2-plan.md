...
- [ ] 导入 20 条网上面经（只有题目），全部 status=unknown，category=knowledge
- [ ] 导入 3 份目标公司 JD，提取技能关键词正确率人工抽查 ≥ 80%
- [ ] 交叉验证后，一道"答错的题"若 topic 命中 JD 关键词，优先级能从 1.0 提升到 ≥ 1.5
- [ ] source 三枚举在检索时可用 `where={"source": ...}` 正确过滤
...
| 里程碑 | 内容                | 产出                             |
| --- | ----------------- | ------------------------------ |
| M1  | 补 v1 收尾           | tag v1.0.0 + issue 修复独立 commit |
| M2  | v1.5 交叉验证         | 导入网上面经 + JD，交叉验证函数 + 验收 4 条    |
| M3  | 攒数据               | 真实面试 ≥ 50 条 KnowledgeItem      |
